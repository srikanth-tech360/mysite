        ...
        <div className="relative">
          ...
          >
            {certifications.map((cert, index) => (
              <div
                key={index}
                className="min-w-[280px] md:min-w-[350px] snap-start bg-white rounded-lg shadow-lg overflow-hidden flex-shrink-0 transition-transform duration-300 hover:transform hover:scale-105"
              >
                <div className="h-2 bg-electric-blue"></div>
          ...
          <div className="flex justify-center mt-8 space-x-2">
            {certifications.map((_, index) => (
              <button
                key={index}
                className="w-3 h-3 rounded-full bg-gray-400 focus:outline-none hover:bg-electric-blue transition-colors duration-300"
                onClick={() => {
